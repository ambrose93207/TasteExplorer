export default async function handler(req, res) {
  ...
  try {
    ...
    const { city, district, food, purpose, budget, people, filters, userTime, mrtStation } = body;
    const GEMINI_API_KEY = process.env.GEMINI_API_KEY;

    // 嚴格遵守要求：使用 gemini-2.5-flash
    ...
    if (mrtStation) {
      ...
      3. 行政區界線彈性：若餐廳鄰近「${mrtStation}」，即使其地址行政區與「${district}」不同，亦優先推薦。
      4. 嚴格核對：必須核對餐廳真實地址，確保距離該站點確實符合 500 公尺內。
      `;
    }

    const prompt = `你是一位台灣美食專家。${timeNote} ${hiddenNote} ${transNote}
      地點：${city}${district} ${mrtStation ? '(鄰近'+mrtStation+')' : ''}
      種類：${food} | 目的：${purpose} | 預算：${budget} | 特殊篩選：${filters}
    ...
    res.status(200).json({ error: `【Vercel 系統報錯】: ${e.message}` });
  }
}
